    ...
    public class GetInfoRepository : IGetInfoRepository
    {
        public string GetInfo(string Division, string addrobj, string house, string room)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                Dictionary<int, ADDROBJModel> addrobjs = new Dictionary<int, ADDROBJModel>(200);

                string CommandText = "";

                if (room != "null")
                {
                    CommandText = "select * from ROOM where HOUSEGUID = '" + room + "'";
                }
                else if (house != "null")
                {
                    CommandText = "select * from HOUSE where AOGUID = '" + house + "'";
                }
                else if (addrobj != null)
                {
                    CommandText = "select * from ADDROBJ where AOGUID = '" + addrobj + "' and ACTSTATUS = " + Division + " order by FORMALNAME";
                }
